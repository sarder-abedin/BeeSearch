...
| **Reviewer** | IEEE-style peer review of one user-selected document. The critique is evidence-backed by **both** the uploaded paper and external scientific literature. Three-step pipeline: (1) extract 3 search queries from the document (topic, methodology, claimed novelty); (2) search arXiv + Semantic Scholar, assign `[E1]`–`[E9]` reference numbers; (3) generate structured review with both doc text and external paper abstracts in context — the LLM cites `[En]` inline wherever external evidence supports or contradicts a critique point (missing baselines, novelty overlaps, mathematical errors, unjustified assumptions). Helper `_build_external_ref_block()` formats the numbered block for the prompt. Sections: Summary / Strengths / Weaknesses / Detailed Critique (Novelty, Technical Soundness [a–d], Experimental Evaluation, Related Work, Writing Quality) / Recommendation (Accept / Minor / Major / Reject). Technical Soundness evaluates: (a) mathematical correctness — equations/derivations/proofs, correct form cited [En]; (b) logical validity — circular reasoning, invalid inference, contradictions; (c) misleading/incorrect claims — cherry-picking, overgeneralisation, contradicts established knowledge, cited [En]; (d) assumption justification — limits cited [En]. Follow-up chat (`reviewer_chat`) is stateless — client sends full `chat_history` and `external_refs` on each call; the chat context includes the same `[E1]`–`[E9]` block so follow-up answers can reference specific papers. Both review generation and chat are backed by background jobs. Uses `ANTI_AI_TELL_REVIEWER_INSTRUCTION` (Wikipedia Signs of AI Writing variant). |
